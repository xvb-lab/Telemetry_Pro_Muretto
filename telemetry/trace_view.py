...
class _LiveMap(QWidget):
    ...
    def _paint_review(self, p):
        ...
        # larghezza REALE della pista (tabella per circuito, dati
        # utente 24/07) convertita con lo zoom; pavimento 16 px per
        # la vista intera (dove 1 m ~ 0.8 px)
        try:
            from data.track_info import width_for_track as _wft9
            _wm9 = float(_wft9(self._track))
        except Exception:
            _wm9 = 15.0
        trk_w = max(16.0, min(300.0,
                              _wm9 * getattr(self, "_ppm9", 1.0)))
        ...
        # macchinina INCHIODATA alla misura reale dell'auto (4.4 m):
        # NON segue piu' la larghezza strada (allargando la
        # carreggiata crescevano pure le macchine, 24/07)
        dot_r = max(2.5, 2.2 * getattr(self, "_ppm9", 1.0))
